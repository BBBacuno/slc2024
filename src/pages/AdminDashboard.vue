<template>
  ...
  <div>
    <q-layout view="lHh Lpr lFf" v-show="$q.localStorage.getItem('logIn')">
      ...
      <q-page-container v-show="$q.localStorage.getItem('logIn') && homeMenu">
        <div class="table-container">
          <q-table
            ...
            :columns="registrant_columns"
            :row-key="registrant_rows.id"
            :pagination="paginationConfig"
            :filter="filter"
          >
            <template v-slot:top-right>
              <q-input borderless dense debounce="300" color="primary" v-model="filter" label="Search" style="margin-right: 50px">
                <template v-slot:append>
                  <q-icon name="search" />
                </template>
              </q-input>
              ...
              <q-btn
                ...
                no-caps
                @click="showUpdateDialog('REGISTER')"
                style="margin-right: 25px"
              />
              <q-btn
                color="primary"
            ...
            <template v-slot:body="props">
              <q-tr :props="props" >
                <q-td>
                  <q-btn
                  color="blue-8"
                  :icon="props.expand ? 'remove' : 'preview'"
                  @click="props.expand = !props.expand"
                  />
                </q-td>
                <q-td>
                  <q-toggle 
                  v-model="props.row.attending"
                  @click="updateAttending(props.row.email, props.row.attending, 'REGISTER')"
                  />
                  <!-- {{props.attending}} -->
                </q-td>
                <q-td
                  v-if="props.cols.name != 'attending'"
                ...
                <q-td align="center">
                  <q-icon
                    :name="props.row.doneEvaluation ? 'check' : 'close'"
                    :color="props.row.doneEvaluation ? 'green' : 'red'"
                    class="cursor-pointer"
                    size="xl"
                  />
                </q-td>
              ...
              <q-tr v-show="props.expand" :props="props">
                <q-td colspan="100%">
                  <div class="text-left">Full Name: {{ props.row.fname + ' ' + props.row.mname + ' ' + props.row.lname + ' ' + props.row.suffix}}</div>
                  ...
                  <div class="text-left"  v-if="(props.row.university != 'null' && props.row.university != null)">University: {{ props.row.university }}</div>
                  ...
                  <div class="text-left"><q-btn @click="showUpdateDialog('UPDATE', props.row)" label="Update Info"/></div>
                </q-td>
              </q-tr>
            </template>
          </q-table>
      ...
      <q-page-container v-show="$q.localStorage.getItem('logIn') && masterListMenu">
        <div class="table-container">
            <q-table
              flat bordered
              title="Master List"
              :rows="masterList_rows"
              :columns="masterList_columns"
              :row-key="masterList_rows.id"
              :pagination="paginationConfig"
              :filter="filter"
            >
              <template v-slot:top-right>
                <q-file v-model="formInput.importMasterList" dense debounce="300" color="primary" label="Import csv" style="margin-right: 50px">
              ...
              <template v-slot:body="props">
                <q-tr :prop="props">
                  ...
                  <!-- <q-td>
                    <q-btn icon="close" color="red-7" @click="deleteMasterListEntry(props.row.email)"/>
                  </q-td> -->
                </q-tr>
              </template>
            </q-table>
      ...
      <q-page-container v-show="$q.localStorage.getItem('logIn') && inOutMenu">
        ...
        >
          <div style="margin: auto; padding: 20px;">
            <q-select
              ...
              v-model="attendanceConduct"
              class="text-fields"
              label="Batch"
              :options="attendanceConduct_options"
            >
              <template v-slot:before>
                <q-icon
                  style="color: #3948ab85"
                  name="school"
                  size="lg"
                />
              </template>
            ...
            <q-select
              ...
              color="indigo-7"
              label-color="indigo-7"
              v-model="attendanceDay"
              class="text-fields"
              label="Conduct"
              :options="attendanceDay_options"
            >
              <template v-slot:before>
                <q-icon
                  style="color: #3948ab85"
        ...
        <div class="table-container">
            <q-table
              ...
              :rows="inOut_rows"
              :columns="inOut_columns"
              :row-key="inOut_rows.id"
              :pagination="paginationConfig"
              :filter="filter"
            >
              <template v-slot:top-right>
                ...
                <q-btn
                  color="primary"
                  icon-right="directions_walk"
                  label="Mass Out"
                  no-caps
                  @click="attendanceMassOut()"
                />
              </template>
            </q-table>
        </div>
      ...
      <q-page-container v-show="$q.localStorage.getItem('logIn') && attendanceMenu">
        <q-card
          ...
          "
          id="topElement"
          :class="attendanceActive ? 'isAttendanceActive' : ''"
        >
          <div>
            <q-input
            v-model="qrcode"
            label="Scan the QR"
            @keydown.enter="checkAttendance(sev)"
            @focus="attendanceActive = true, ranNum = Math.floor(Math.random() * 41)"
            @blur="attendanceActive = false, ranNum = Math.floor(Math.random() * 41)"
            style="
              opacity: 0;
              position: absolute;
              height: 80%;
          ...
          <transition-group
          appear
          :enter-active-class="'animated ' + randomEntrance[ranNum]"
          :leave-active-class="'animated ' + randomExit[ranNum]"
          >
            <div v-if="attendanceActive == true && attendanceChecked == false" class="attendance-container" style="position: absolute; z-index:-100">
            ...
            <div v-if="attendanceActive == false && attendanceChecked == false" class="attendance-container" style="position: absolute; z-index:-100">
              <span class="attendance-text">
                <h1>
                  Click here
                </h1>
            ...
            <div v-if="attendanceChecked == 'FIRST'" class="attendance-container" style="position: absolute; z-index:-100">
              <span class="attendance-text">
                <h1>
                  Hello {{ attendanceName }},
                  <br><q-icon name="celebration" size="xl" color="indigo-7"/> <q-icon name="celebration" size="xl" color="indigo-7"/> <q-icon name="celebration" size="xl" color="indigo-7"/><br>
            ...
            <div v-if="attendanceChecked == 'qrUnread'" class="attendance-container" style="position: absolute; z-index:-100">
              <span class="attendance-text">
                <h1>
            ...
            <div v-if="attendanceChecked == 'selectNull'" class="attendance-container" style="position: absolute; z-index:-100">
              <span class="attendance-text">
                <h1>
            ...
            <div v-if="attendanceChecked == 'regionMismatch'" class="attendance-container" style="position: absolute; z-index:-100">
              <span class="attendance-text">
                <h1>
      ...
      <q-page-container v-show="$q.localStorage.getItem('logIn') && pretestMenu">
        <div class="table-container">
          <q-table
            title="Pre-Test Responses"
            :rows="pretest_rows"
            :columns="pretest_columns"
            :row-key="pretest_rows.id"
            :pagination="paginationConfig"
            :filter="filter"
          >
            <template v-slot:top-right>
        ...
        <template v-slot:body-cell-evaldone="props">
          <q-td>
            <q-td align="center">
              <!-- {{ props.row.doneeval }} -->
              <q-icon
                :name="props.row.doneeval ? 'check' : 'close'"
                :color="props.row.doneeval ? 'green' : 'red'"
                class="cursor-pointer"
                size="xl"
              />
            </q-td>
    ...
    <!-- <q-dialog v-model="updateOldEmail" persistent>
      <q-card style="min-width: 350px" class="prompt">
        <q-card-section class="q-pb-none text-center">
          <q-icon style="color: #3948ab85" name="error" size="70px" />
        </q-card-section>
        ...
        <q-card-section
          ...
          style="font-family: Montserrat; font-size: 15px"
        >
          <div v-for="entry in oldEmails" :key="entry.name">
            <q-btn
            :label="entry.email"
            @click="renewEmail(entry.email)"
    ...
    <!-- <q-dialog v-model="renewEmailTab" persistent>
      <q-card style="min-width: 350px" class="prompt">
        <q-card-section class="q-pb-none text-center">
          <q-icon style="color: #3948ab85" name="error" size="70px" />
        </q-card-section>
    ...
    <q-dialog v-model="exportDialog">
      <q-card style="min-width: 650px" class="prompt">
        <q-card-section class="q-pa-md text-center uploads-container">
            <q-select 
            v-model="exportByRegion"
            label="Region(s)"
            multiple
            :options="exportByRegion_column"
            />
            <div class="button-container">
          ...
          <div class="form-container">
            <div v-show="pageNum == 1">
              <div class="left-container">
                ...
                <q-input
                  @input= "val => { formInput.fName = val.toUpperCase() }"
                  outlined
                  rounded
                  v-model="formInput.fName"
                  :color="formColor"
                  :label-color="formColor"
                  label="First Name"
                ...
                <q-input
                  @input= "val => { formInput.mName = val.toUpperCase() }"
                  outlined
                  rounded
                  v-model="formInput.mName"
                  :color="formColor"
                  :label-color="formColor"
                  label="Middle Name"
                  hint="Put N/A if not applicable"
                  :rules="[(val) => val && val.length > 0 || 'Required']"
                  lazy-rules
                >
                  <template v-slot:before>
                ...
                <q-input
                  @input= "val => { formInput.lName = val.toUpperCase() }"
                  outlined
                  rounded
                  v-model="formInput.lName"
                  :color="formColor"
                  :label-color="formColor"
                  label="Last Name"
                  :rules="[(val) => val && val.length > 0 || 'Required']"
              ...
              <div class="right-container">
                ...
                >
                  ...
                  <template v-slot:append>
                    <q-icon name="close" @click="formInput.suffix=null" class="remove-selection" v-show="formInput.univCity"></q-icon>
                  </template>
                </q-select>
                <q-input
                  outlined
                  ...
                  @click="$refs.birthdateProxy.show()"
                  :rules="[
                ...
                >
                  ...
                  <template v-slot:append>
                    <q-icon
                      style="color: #3948ab85; margin-right: 10px"
                      name="event"
                      class="cursor-pointer"
                    >
                      <q-popup-proxy
            ...
            <div v-show="pageNum == 2">
              <div class="radios">
                ...
                >
                  <q-img
                    src="/radio-option1.png"
                    alt=""
                    srcset=""
                    @click="selectParticipantType(1)"
                ...
                >
                  <q-img
                    src="/radio-option2.png"
                    alt=""
                    srcset=""
                    @click="selectParticipantType(2)"
              ...
              <div class="left-container">
                <q-select
                  ...
                  :color="formColor"
                  :label-color="formColor"
                  v-model="formInput.univRegion"
                  label="Region of your School"
                ...
                <q-slide-transition :duration="700">
                  ...
                  <q-select
                    ...
                    :color="formColor"
                    :label-color="formColor"
                    v-model="formInput.univCity"
                    use-input
                    emit-value
                    map-options
                    input-debounce="0"
                    label="City of your University"
                    :options="filtered_univCity"
                    @filter="filter_univCity"
                    behavior="menu"
                    lazy-rules
                    :rules="[
                  ...
                  >
                    ...
                    <template v-slot:append>
                      <q-icon name="close" @click="formInput.univCity=null" class="remove-selection" v-show="formInput.univCity"></q-icon>
                    </template>
                  </q-select>
                    <q-select
                      outlined
                      ...
                      :color="formColor"
                      :label-color="formColor"
                      v-model="formInput.university"
                      use-input
                      emit-value
                      map-options
                      input-debounce="0"
                      label="University"
                      :options="filtered_university"
                      @filter="filter_university"
                      behavior="menu"
                      lazy-rules
                      :rules="[
                        (val) => (val && val > 0) || 'Required Field',
                    ...
                    >
                      ...
                      <template v-slot:append>
                        <q-icon name="close" @click="formInput.university=null" class="remove-selection" v-show="formInput.university"></q-icon>
                      </template>
                    </q-select>
                  </div>
                </q-slide-transition>
                ...
                <q-slide-transition :duration="700">
                  <div v-show="formInput.participant == 1">
                    <q-select
                      ...
                      :color="formColor"
                      :label-color="formColor"
                      v-model="formInput.course"
                      use-input
                      emit-value
                      map-options
                      input-debounce="0"
                      label="Course"
                      :options="filtered_course"
                    ...
                    >
                      ...
                      <template v-slot:append>
                        <q-icon name="close" @click="formInput.course=null" class="remove-selection" v-show="formInput.course"></q-icon>
                      </template>
                    </q-select>
                    <q-select
                      outlined
                    ...
                    <q-select
                      ...
                      :color="formColor"
                      :label-color="formColor"
                      v-model="formInput.scholarProgram"
                      label="Scholarship Program"
                      :options="scholarProgram_options"
                      emit-value
                      map-options
                      lazy-rules
                      :rules="[
                    ...
                    <q-select
                      ...
                      :color="formColor"
                      :label-color="formColor"
                      v-model="formInput.yearAward"
                      label="Year Award"
                      :options="yearAward_options"
                      emit-value
                      map-options
                      lazy-rules
                      :rules="[
                    ...
                    <q-input
                      outlined
                      rounded
                      v-model="formInput.association"
                      :color="formColor"
                      :label-color="formColor"
                      label="Associations"
                      hint="e.g Member/President of Scholars Club"
            ...
            <div class="button-container">
              ...
              <div class="button-container" style="width: 100%;" v-show="pageNum == 2 || (pageNum == 2 && formInput.participant == 2)">
                <q-btn label="submit" type="submit" color="primary" class="button-submit"  @click="submitResponse(action, idNum), navigationToggle('home')" v-close-popup />
              </div>
      ...
      <q-card style="min-width: 950px" class="prompt">
        <q-card-section>
          <div class="form-container">
            ...
            <div class="button-container">
              <q-btn label="submit" type="submit" color="primary" class="button-submit"  @click="submitResponse(action, idNum), navigationToggle('masterList')" v-close-popup />
            </div>
          </div>
        </q-card-section>
      </q-card>
