<template>
  ...
  <q-card class="login-container"
    v-show="$q.localStorage.getItem('logIn') == false || $q.localStorage.getItem('logIn') == null">
    <q-form>
      <q-card-section>
  ...
  <q-layout view="lHh Lpr lFf" v-show="$q.localStorage.getItem('logIn')">
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above bordered>
      <q-list>
        ...
        <q-item>
          <q-btn @click="navigationToggle('home')" class="sidebar-button" flat rounded align="left">
            <q-icon left name="home" />
            <div>Home</div>
          </q-btn>
        </q-item>
        <q-item>
          <q-btn @click="navigationToggle('masterList')" class="sidebar-button" flat rounded align="left">
            <q-icon left name="checklist" />
            <div>Master List</div>
          </q-btn>
        </q-item>
        <q-item>
          <q-btn @click="navigationToggle('attendance')" class="sidebar-button" flat rounded align="left">
            <q-icon left name="groups_2" />
            <div>Attendance</div>
          </q-btn>
        </q-item>
        <q-item>
          <q-btn @click="navigationToggle('inOut')" class="sidebar-button" flat rounded align="left">
            <q-icon left name="double_arrow" />
            <div>In and Out</div>
          </q-btn>
        </q-item>
        <q-item>
          <q-btn @click="navigationToggle('pretest')" class="sidebar-button" flat rounded align="left">
            <q-icon left name="quiz" />
            <div>Pre-Test Responses</div>
          </q-btn>
        </q-item>
        <q-item>
          <q-btn @click="navigationToggle('eval')" class="sidebar-button" flat rounded align="left">
        ...
        <q-item /><q-item />
        <q-item>
          <q-btn label="Log Out" @click="logOutUser()" class="sidebar-button" />
        </q-item>
      ...
      <div class="table-container">
        <q-table title="Registrants" :rows="registrant_rows" :columns="registrant_columns" :row-key="registrant_rows.id"
          ...
          <template v-slot:top-right>
            ...
            <q-btn color="primary" icon-right="boy" label="Manual Register" no-caps @click="showUpdateDialog('REGISTER')"
          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              <q-td>
                <q-btn color="blue-8" :icon="props.expand ? 'remove' : 'preview'" @click="props.expand = !props.expand" />
              </q-td>
              <q-td>
        ...
        <q-table flat bordered title="Who's In, Who's Out?" :rows="inOut_rows" :columns="inOut_columns"
          ...
          <template v-slot:top-right>
            ...
            <q-btn color="primary" icon-right="archive" label="Generate Groupings" no-caps
              @click="generateGroupings(inOut_columnsExport, inOut_rows, 'Attendance-Groupings')"
              style="margin-right: 25px" />
            ...
            <div class="radios" style="width: 100%;">
              ...
              <div v-show="pageNum < 2 && !(pageNum == 2 && formInput.participant == 2)" style="width: 25%;">
                <q-btn label="Next" @click="pageNum++, scrollToElement('#topElement')" color="teal" class="button-submit"
                  style="width: 100%; height: 12%"></q-btn>
              </div>
            </div>
...
const homeMenu = ref(true)
const pretestMenu = ref(false)
const inOutMenu = ref(false)
...
const attendanceConduct = ref(null)
const attendanceDay = ref(null)
  ...
  setup() {
    ...
    return {
      ...
      generateGroupings(column, row, filename) {
        ...
        this.exportTable(column, row, attendanceConduct.value, filename)
